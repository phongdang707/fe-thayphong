...
export const initialState = {
  ...
  grade: { error: null, value: "10", listErros: [] },
  ...
  produce(state, (draft) => {
    switch (action.type) {
      ...
      case Types.SUBMIT_ADD_STUDENT_DONE: {
        ...
        draft.isSucess = true;
        draft.isError = false;
        draft.noti.open = !draft.noti.open;
        draft.noti.type = 'success';
        draft.noti.content = 'Gửi mẫu đăng ký thành công'
        return;
      }
      case Types.SUBMIT_ADD_STUDENT_ERROR: {
        ...
        draft.isLoading = false;
        draft.name.error = true;
        draft.name.listErros = [data];
        draft.noti.open = !draft.noti.open;
        draft.noti.type = 'error';
        draft.noti.content = data;
