...
import {downloadFileDone, viewDeleteError, viewDeleteSuccess, viewError, viewSuccess,} from "./action";
...
export default function* watcherSaga() {
    ...
    yield takeLatest(Types.CONFIRM_STUDENT, onConfirmStudent);
    yield takeLatest(Types.REFUSE_STUDENT, onRefuseStudent);
}
...
export function* viewCourseRequest(action) {
    try {
        const data = yield select(makeSelectData());
        const postData = mapData(data);
        const res = yield call(services.students.getListStudentConfirm, postData);
        if (res.status === 200) {
...
export function* onConfirmStudent(action) {
    try {
        const data = yield select(makeSelectData());
        const postData = mapDataDelete(data);
        const res = yield call(services.students.confirmStudentToUser, postData);
        if (res.status === 200) {
            yield put(viewDeleteSuccess());
        }
    } catch (err) {
...
export function* onRefuseStudent(action) {
    try {
        const data = yield select(makeSelectData());
        const postData = mapDataDelete(data);
        const res = yield call(services.students.deleteStudentRegister, postData);
        if (res.status === 200) {
            yield put(viewDeleteSuccess());
        }
    } catch (err) {
